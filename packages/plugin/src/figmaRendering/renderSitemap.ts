...
import { createScreenshotPages, updateNavigationLinks } from "./utils/createScreenshotPages";

async function createIndexPage(tree: TreeNode, pageIdMap: Map<string, string>): Promise<string> {
  ...
  async function createIndexEntry(node: TreeNode, depth: number = 0): Promise<TextNode[]> {
