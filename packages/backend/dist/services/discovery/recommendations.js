...
function hasLanguagePrefix(path) {
...
function isLanguageDuplicate(candidate, selected) {
    if (!hasLanguagePrefix(candidate.path))
        return false;
    const stripped = stripLanguagePrefix(candidate.path);
    return selected.some((s) => {
        if (s.pageType !== candidate.pageType)
            return false;
...
function computeBaseScore(candidate) {
    let score = 0;
    // Source signals
    if (candidate.reasons.includes("seed-url"))
        score += 40;
    if (candidate.reasons.includes("main-nav"))
        score += 30;
    if (candidate.reasons.includes("footer"))
        score += 20;
    // Page type value
    if (VALUABLE_TYPES.has(candidate.pageType))
        score += 20;
    // Penalties
    if (candidate.pageType === "legal" || candidate.pageType === "utility")
        score -= 40;
    if (candidate.pageType === "support-doc")
        score -= 30;
    return score;
}
export function generateRecommendations(input) {
    ...
    const recommended = [];
    const selectedPatternKeys = new Set();
    ...
    while (recommended.length < pageBudget && mutable.length > 0) {
        ...
        if (!pick)
            break;
        // Check hard caps
        const cap = pick.pageType === "blog-article" && !includeBlog
            ? 0
