...
function startInventoryBuild(projectId, workspacePath) {
...
function completeInventoryBuild(buildId, data) {
...
function failInventoryBuild(buildId) {
    db.update(inventoryBuilds)
...
export async function buildWorkspaceFromData(projectId, data, options = {}) {
    ...
    try {
        ...
        if (buildRecord)
            completeInventoryBuild(buildRecord.id, data);
    }
    catch (error) {
        if (buildRecord)
            failInventoryBuild(buildRecord.id);
        throw error;
    }
    return {
        projectId,
