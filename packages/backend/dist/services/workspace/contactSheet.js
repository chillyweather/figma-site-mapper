...
export async function buildContactSheet(category, groups, outPath) {
    const cols = 6;
    const cellW = 220;
    const cellH = 140;
    const pad = 8;
    ...
    if (visibleGroups.length === 0) {
        ...
        await sharp(Buffer.from(`<svg width="${cellW}" height="${cellH}" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="white"/><rect x="0.5" y="0.5" width="${cellW - 1}" height="${cellH - 1}" fill="none" stroke="#e5e7eb"/><text x="16" y="72" font-size="13" font-family="Arial, sans-serif" fill="#64748b">No crop-backed ${escapeXml(category)} groups</text></svg>`))
            .png()
            .toFile(outPath);
    ...
    for (let i = 0; i < visibleGroups.length; i++) {
        ...
        composites.push({
            input: Buffer.from(`<svg width="${cellW}" height="24" xmlns="http://www.w3.org/2000/svg"><rect width="${cellW}" height="24" fill="white" fill-opacity="0.9"/><text x="6" y="16" font-size="11" font-family="Arial, sans-serif" fill="#111827">${escapeXml(label)}</text></svg>`),
...
export async function buildContactSheets(workspacePath, groupsByFolder) {
    for (const [folder, groups] of groupsByFolder.entries()) {
        await buildContactSheet(folder, groups, path.join(workspacePath, "catalog", folder, "contact-sheet.png"));
    }
}
