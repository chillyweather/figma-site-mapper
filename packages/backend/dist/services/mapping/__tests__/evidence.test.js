...
const makeDomCandidate = (overrides = {}) => ({
    id: "1",
    pageId: "10",
    type: "button",
    classes: [],
...
describe("filterDomCandidates", () => {
    ...
    it("excludes elements with zero-width bbox", () => {
        expect(filterDomCandidates([makeRow({ bbox: JSON.stringify({ x: 0, y: 0, width: 0, height: 40 }) })])).toHaveLength(0);
    });
    it("excludes elements with zero-height bbox", () => {
        expect(filterDomCandidates([makeRow({ bbox: JSON.stringify({ x: 0, y: 0, width: 100, height: 0 }) })])).toHaveLength(0);
    });
