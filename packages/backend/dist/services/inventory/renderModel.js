...
function workspaceRelativePathExists(workspacePath, relativePath) {
    if (!relativePath || relativePath.includes("undefined"))
        return false;
...
function existingWorkspaceAssetUrl(workspacePath, baseUrl, projectId, relativePath) {
    if (!workspaceRelativePathExists(workspacePath, relativePath))
        return null;
    if (!relativePath)
        return null;
    return workspaceAssetUrl(baseUrl, projectId, normalizeRelativeWorkspacePath(relativePath));
}
function elementToExample(element, workspacePath, projectId, baseUrl) {
...
function scoreElementForCluster(cluster, element) {
    const clusterText = [
    ...
    if (element.visible !== false)
        score += 5;
    if (clusterText.includes("button") && category === "button")
        score += 45;
    if (clusterText.includes("link") && category === "link")
        score += 45;
    if (clusterText.includes("heading") && category === "heading")
        score += 45;
    if ((clusterText.includes("logo") || clusterText.includes("image")) && category === "image")
        score += 45;
    if ((clusterText.includes("stats") || clusterText.includes("text-block")) && category === "text-block")
        score += 35;
    if (clusterText.includes("primary") && text.includes("start free"))
        score += 120;
    if (clusterText.includes("outline") && text.includes("contact sales"))
        score += 120;
    if ((clusterText.includes("demo") || clusterText.includes("request")) && text.includes("request a demo"))
        score += 120;
    if (clusterText.includes("nav-trigger") && region === "top" && category === "button")
        score += 90;
    if (clusterText.includes("chip") && (category === "button" || category === "link") && text.length > 0 && text.length <= 40)
        score += 60;
    if (clusterText.includes("final-cta") && (elementText.includes("ready to see") || elementText.includes("runs on atera")))
        score += 120;
    if (clusterText.includes("global.header") && (region === "top" || element.isGlobalChrome === true))
        score += 95;
    if (clusterText.includes("global.footer") && (region === "bottom" || element.isGlobalChrome === true))
        score += 85;
    if (clusterText.includes("faq") && (text.includes("what is") || text.includes("faq")))
        score += 120;
    if (clusterText.includes("stats") && /\b\d+([.,]\d+)?\s*(%|k|x|min|hrs?)?\b/i.test(text))
        score += 110;
    if (clusterText.includes("logo") && category === "image")
        score += 100;
    if (clusterText.includes("product-summary") && (elementText.includes("robin") || elementText.includes("essentials suite")))
        score += 120;
    if (clusterText.includes("section-title") && category === "heading" && (tag === "h2" || tag === "h3"))
        score += 90;
    if (clusterText.includes("eyebrow") && text.length > 0 && text.length <= 60 && (category === "heading" || category === "text-block"))
        score += 75;
    if (clusterText.includes("read-case-study") && text.includes("read case study"))
        score += 120;
    if (clusterText.includes("learn-more") && text.includes("learn more"))
        score += 120;
    return score;
}
function fallbackExamplesForCluster(cluster, elements, workspacePath, projectId, baseUrl, existingIds) {
    return elements
...
export async function buildInventoryRenderModel(projectId, baseUrl) {
    ...
    for (const entry of pageEntries) {
        ...
        for (const element of pageElements) {
            ...
            if (id)
                elementsById.set(id, enrichedElement);
    ...
    for (const cluster of clusters) {
        ...
        if (examples.length === 0 || examples.every((example) => !example.cropUrl && !example.cropContextUrl)) {
            const existingIds = new Set(examples.map((example) => example.elementId).filter((id) => Boolean(id)));
