...
function classifyKind(cluster) {
    const background = cluster.signature.backgroundColor;
    const border = cluster.signature.borderColor;
    const transparentBackground = !background || background === "transparent" || background === "none";
...
function classifyHeadingLevel(cluster) {
    const fontSizeValue = Number.parseInt(cluster.signature.fontSize || "", 10);
    if (!fontSizeValue || Number.isNaN(fontSizeValue))
        return "unknown";
    if (fontSizeValue >= 40)
        return "display";
    if (fontSizeValue >= 32)
...
function buildFamilyKey(cluster) {
    return [
        cluster.category,
...
function inferVariantAxes(category, cluster, familyClusters) {
    const axes = [];
    ...
    if (category === "heading") {
        const levels = Array.from(new Set(familyClusters.map(classifyHeadingLevel))).filter((value) => value !== "unknown");
        if (levels.length > 1) {
            axes.push({
                name: "level",
                values: levels,
...
function buildVariantHints(variantAxes) {
...
export function analyzeComponentClusters(elements) {
    ...
    const rawClusters = Array.from(accumulators.values()).filter((cluster) => cluster.members.length >= 2);
