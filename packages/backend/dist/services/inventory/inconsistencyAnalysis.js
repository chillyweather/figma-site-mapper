function sharedSignatureFields(a, b) {
...
export function analyzeInconsistencies(tokens, clusters) {
    ...
    for (let i = 0; i < largeClusters.length; i++) {
        for (let j = i + 1; j < largeClusters.length; j++) {
            ...
            if (similarity < 0.65)
                continue;
            const impactScore = Number((similarity * (left.instanceCount + right.instanceCount) * 0.5).toFixed(2));
