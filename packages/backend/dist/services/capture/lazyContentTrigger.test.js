...
describe("LazyContentTrigger", () => {
    ...
    it("scrolls the page so all lazy-loaded images end up loaded", async () => {
        ...
        const beforeLoaded = await page.evaluate(() => document.querySelectorAll("img").length === 0
            ? 0
        ...
        await triggerLazyContent(page);
        const afterLoaded = await page.evaluate(() => Array.from(document.images).filter((img) => img.complete && img.naturalWidth > 0).length);
