...
import { applyStealthContextDefaults, getStealthLauncher, pickStealthUserAgent, STEALTH_HTTP_HEADERS, } from "./stealthLauncher.js";
async function probeBrowser(browser, baseUrl, options) {
    ...
    const probe = await page.evaluate(async () => {
        ...
        const chromeRuntime = Boolean(window.chrome?.runtime);
        ...
        try {
            ...
            if (gl) {
                ...
                if (debugInfo) {
                    webglVendor = String(gl.getParameter(debugInfo.UNMASKED_VENDOR_WEBGL));
