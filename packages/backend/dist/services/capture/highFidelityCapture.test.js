...
describe("HighFidelityCapture", () => {
    ...
    it("captures a page combining lazy images + web font + animation + sticky banner", async () => {
        ...
        const loadedCount = await page.evaluate(() => Array.from(document.images).filter((i) => i.complete && i.naturalWidth > 0).length);
        expect(loadedCount).toBe(4);
        // 5. Sticky banner: only the topmost should remain visible.
        const visibility = await page.evaluate(() => {
            const b1 = document.querySelector('[data-testid="banner-1"]');
            const b2 = document.querySelector('[data-testid="banner-2"]');
            return {
    ...
    it("preserves tall scroll-pinned hero layers (atera-style)", async () => {
        ...
        const visibility = await page.evaluate(() => {
            ...
            const heroBg = document.querySelector('[data-testid="hero-bg"]');
            const heroText = document.querySelector('[data-testid="hero-text"]');
            return {
