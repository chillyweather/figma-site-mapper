...
export async function captureTiled(page, opts = {}) {
    const dismiss = opts.dismissBanners ?? (async () => undefined);
    const progress = opts.onProgress ?? ((_s) => undefined);
    ...
    const { scrollHeightCss, viewportHeightCss, viewportWidthCss, dpr } = await getPageDimensions(page);
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        const tileSurfaces = preSurfaces.filter((s) => s.bbox.y < tileBottom && s.bbox.y + s.bbox.height > tile.topCss);
        ...
        const mediaRegionsInTile = tileSurfaces.filter((s) => s.kind === "canvas" || s.kind === "svg-animation" || s.kind === "lottie" || s.kind === "dotlottie");
...
function planTiles(scrollHeightCss, tileHeightCss) {
    if (scrollHeightCss <= 0)
        return [{ topCss: 0 }];
    const tiles = [];
    let y = 0;
...
async function getPageDimensions(page) {
    const dims = await page
        .evaluate(() => ({
        scrollHeightCss: Math.max(document.documentElement.scrollHeight, document.body.scrollHeight, document.documentElement.offsetHeight),
...
async function prepareStickyElementsForTiling(page) {
    await page
        .evaluate(({ maxBandHeight, maxTop }) => {
        const candidates = [];
        ...
        all.forEach((el) => {
            ...
            if (!Number.isFinite(topPx) || topPx > maxTop || rect.top > maxTop)
                return;
            candidates.push(el);
        });
...
async function setStickyBandsVisibleForTile(page, visible) {
    await page
        .evaluate((show) => {
        document
            .querySelectorAll("[data-sitemapper-tiled-sticky-band='true']")
            .forEach((el) => {
            if (!show) {
...
async function discoverSurfacesForActivation(page) {
    const raw = await page
        ...
        document.querySelectorAll("video").forEach((el, i) => {
            const b = rect(el);
            if (b)
                results.push({ kind: "video", selector: `video:nth(${i})`, bbox: b, status: "pending" });
        });
        document.querySelectorAll("canvas").forEach((el, i) => {
            const b = rect(el);
            if (b && b.width * b.height >= 400)
                results.push({ kind: "canvas", selector: `canvas:nth(${i})`, bbox: b, status: "pending" });
        });
        document.querySelectorAll("svg").forEach((el, i) => {
            ...
            const b = rect(el);
            if (b && b.width * b.height >= 400)
                results.push({ kind: "svg-animation", selector: `svg:nth(${i})`, bbox: b, status: "pending" });
        });
        ["lottie-player", "dotlottie-player", "[data-lottie]"].forEach((sel) => {
            document.querySelectorAll(sel).forEach((el, i) => {
                const b = rect(el);
                if (b)
                    results.push({ kind: sel.includes("dotlottie") ? "dotlottie" : "lottie", selector: `${sel}:nth(${i})`, bbox: b, status: "pending" });
            });
        });
        return results;
    })
...
async function activateTileMedia(page, surfaces) {
...
async function scoreTileMediaRegions(tileBuf, surfaces, tileCssTop, dpr) {
    try {
        ...
        for (const surface of surfaces) {
            ...
            if (x < 0 || y < 0 || x + w > tileW || y + h > tileH || w < 4 || h < 4)
                continue;
