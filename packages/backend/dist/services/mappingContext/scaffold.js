...
import { getMappingInputs } from "../mappingInputs.js";
...
function hasEvidence(inputs) {
    return Boolean(inputs.repoPath ||
        inputs.branchName ||
        inputs.storybookUrl ||
...
function toProfile(projectId, inputs, repo) {
    return {
        projectId,
...
function buildEvidence(repoIndex, storybookIndex, storybookComponents) {
    return {
...
function logContent(projectId, inputs, generatedAt, repoLogLines, storybookLogLines) {
    const lines = [
        "# Mapping Context Log",
        "",
...
export async function writeMappingContextScaffold(workspacePath, projectId, generatedAt) {
    ...
    const repoResult = await analyzeRepoMappingInput(inputs.repoPath, inputs.branchName, generatedAt, inputs.tokenSources);
    const storybookResult = await analyzeStorybookMappingInput(generatedAt, inputs.storybookPath, inputs.storybookUrl, repoResult.repo.path ?? "", repoResult.repoIndex);
    ...
    const suggestions = buildSuggestions(repoResult.repoIndex, storybookResult.storybookIndex, {
    ...
    await Promise.all([
        ...
        writeJson(path.join(mappingContextPath, "evidence.json"), buildEvidence(repoResult.repoIndex, storybookResult.storybookIndex, storybookResult.evidenceComponents)),
    ...
    await import("fs").then((fs) => fs.promises.writeFile(path.join(mappingContextPath, "log.md"), logContent(projectId, inputs, generatedAt, repoResult.logLines, storybookResult.logLines), "utf8"));
}
