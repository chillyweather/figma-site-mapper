...
function parseTitle(content) {
    ...
    if (mdxTitle?.[1])
        return mdxTitle[1].trim();
    return null;
}
function parseComponentName(content, title) {
...
function parseVariantNames(content) {
    const variants = [];
...
function parseArgNames(content) {
    const names = new Set();
...
function parseStoryFile(rootPath, filePath) {
...
function parseStaticStorybookIndex(rootPath, jsonPath) {
    try {
...
function collectStoriesFromDirectory(rootPath, scanRoot, maxDepth = 6) {
...
function toEvidenceComponents(stories, sourceLabel) {
...
async function fetchStorybookIndexFromUrl(url) {
...
export async function analyzeStorybookMappingInput(generatedAt, configuredStorybookPath, configuredStorybookUrl, repoPath, repoIndex) {
    ...
    const componentCount = new Set(normalizedStories.map((story) => story.componentName).filter(Boolean)).size;
    const sourceLabel = sourceMode === "configured-path"
        ? "storybook-path"
        : sourceMode === "configured-url"
            ? "storybook-url"
            : "storybook-repo";
