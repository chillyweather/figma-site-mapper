...
async function main() {
    ...
    const clustersRaw = await readJson(clustersPath);
    if (!clustersRaw) {
    ...
    for (const [id, count] of clusterIds.entries()) {
        if (count > 1)
            errors.push(`Duplicate cluster ID: ${id} (${count} occurrences)`);
    }
    // Load catalog groups
    ...
    for (const folder of folders) {
        const groups = await readJson(path.join(catalogDir, folder, "groups.json"));
