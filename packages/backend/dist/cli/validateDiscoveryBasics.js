...
function main() {
    ...
    for (const c of classifyCases) {
        ...
        const okType = assertEqual(result.pageType, c.expectedType, `classify("${c.path}").pageType`);
        const okPattern = assertEqual(result.patternKey, c.expectedPattern, `classify("${c.path}").patternKey`);
        if (okType && okPattern) {
            passed++;
        }
        else {
