...
function qualityStatusFromResult(score, retryCount, threshold = 0.1) {
    if (retryCount === 0)
        return score > threshold ? "suspicious" : "clean";
    return score > threshold ? "retry_unchanged" : "retry_improved";
}
function signalsLine(r) {
    return Object.entries(r)
        .map(([k, v]) => `${k[0]}=${v === "fired" ? "✓" : "✗"}`)
        .join(" ");
}
// ── Single capture ────────────────────────────────────────────────────────────
async function smokeOne(url, group, runIndex) {
    ...
    try {
        ...
        try {
            ...
            const qualityStatus = qualityStatusFromResult(result.suspiciousRegionScore, result.retryCount);
            return {
                url,
                group,
                runIndex,
                ...
                qualityStatus,
                readinessSignals: result.readinessReport,
            };
        }
        finally {
...
async function compareConsistency(run1, run2) {
    ...
    const pixelDiff = scoreDelta < 0.05 ? "consistent" : scoreDelta < 0.15 ? "minor-variance" : "inconsistent";
...
async function main() {
    ...
    const matrix = [
        ...ATERA_URLS.map((url) => ({ url, group: "atera" })),
        ...extraAtera.map((url) => ({ url, group: "atera" })),
        ...CONTROL_URLS.map((url) => ({ url, group: "control" })),
    ];
    ...
    for (let i = 0; i < matrix.length; i++) {
        ...
        for (const runIndex of [0, 1]) {
            ...
            if (run.error) {
                ...
                process.stdout.write(` ${run.qualityStatus === "clean" || run.qualityStatus === "retry_improved" ? "✅" : "⚠️ "} ` +
        ...
        if (runs[0] && runs[1] && !runs[0].error && !runs[1].error) {
            ...
            process.stdout.write(`  consistency: ${pixelDiff} (score delta=${scoreDelta.toFixed(3)})\n`);
        }
    ...
    for (const [url, runs] of runsByUrl) {
        ...
        const overallOk = errors === 0 &&
            statuses.every((s) => s === "clean" || s === "retry_improved");
        const icon = overallOk ? "✅" : "⚠️ ";
        process.stdout.write(`${icon} ${label.padEnd(10)} ${url}\n` +
...
main().catch((error) => {
    process.stderr.write(`smoke matrix failed: ${error instanceof Error ? error.stack : String(error)}\n`);
    process.exit(1);
});
