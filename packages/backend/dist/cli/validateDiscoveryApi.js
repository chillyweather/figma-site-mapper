...
async function main() {
    ...
    check(assertTrue(startBody.recommended.every((c) => c.id && c.id !== "0"), "recommended candidates have persisted DB ids"));
    ...
    check(assertTrue(getBody.candidates.some((c) => c.isRecommended), "GET candidates include persisted recommendations"));
    // Test invalid runId
    ...
    const candidateIds = getBody.candidates
        .filter((c) => c.pageType === "homepage" || c.pageType === "pricing")
        .map((c) => c.id);
    ...
    if (typeof narrowedCandidateId === "string") {
        ...
        check(assertEqual(narrowedApprovalBody.approvedUrls.length, 1, "second approval replaces previous approved set"));
        const narrowedDb = db
            .select()
            .from(discoveryCandidates)
            .where(eq(discoveryCandidates.discoveryRunId, parseInt(runId, 10)))
            .all();
        check(assertEqual(narrowedDb.filter((c) => c.isApproved).length, 1, "DB approved candidates are replaced, not accumulated"));
    }
    // Cleanup
