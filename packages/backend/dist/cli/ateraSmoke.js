    ...
    for (const url of MATRIX_URLS) {
        for (let run = 1; run <= RUNS; run++) {
            ...
            try {
                ...
                console.log(`  → ${result.width}x${result.height}px, ${result.tileCount} tiles, ` +
                ...
                fs.writeFileSync(path.join(OUT_DIR, `${url.replace(/[^a-z0-9]/gi, "_").slice(0, 50)}_run${run}_ERROR.txt`), String(err));
            }
            finally {
                await context.close();
            }
