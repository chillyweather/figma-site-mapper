 ...
 */
async function detectInteractiveElementsOnPage(
  page: any,
  log: any
  ...
  crawler = new PlaywrightCrawler(
    {
      ...
      async requestHandler({ request, page, log, enqueueLinks }) {
        ...
        try {
          ...
          log.info(
            `Could not grant permissions: ${error instanceof Error ? error.message : String(error)}`
          );
        }
        ...
        const captchaIndicators = await page.evaluate(() => {
          ...
          const hasElements = captchaSelectors.some((selector) =>
          ...
          const hasCaptchaTitle = captchaTexts.some((text) =>
        ...
        if (detectInteractiveElements && captureOnlyVisibleElements) {
          ...
          log.info(
            `Finding initially visible interactive elements on ${request.url}`
          );
          interactiveElements = await detectInteractiveElementsOnPage(
            page,
            log
          );
        }
