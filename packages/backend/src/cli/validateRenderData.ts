...
import type { InventoryRenderData, RenderCard } from "@sitemapper/shared";
...
async function main(): Promise<void> {
  ...
  let payload: InventoryRenderData;
  try {
    payload = await buildInventoryRenderData(projectId, "http://localhost:3006") as unknown as InventoryRenderData;
  } catch (error) {
  ...
  if (!schemaResult.success) {
    errors.push(
      `Schema validation failed: ${schemaResult.error.issues.map((i) => `${i.path.join(".")} — ${i.message}`).join("; ")}`
    );
  }
