...
async function smokeOne(url: string): Promise<Result> {
...
function format(result: Result, index: number, total: number): string {
  const header = `[${index + 1}/${total}] ${result.url}  (${result.durationMs}ms)`;
  const lines: string[] = [header, `  → kind: ${result.kind}`];
...
async function main(): Promise<void> {
  ...
  if (urls.length === 0) {
    process.stderr.write(
      "Usage: pnpm --filter backend crawl:smoke <url> [<url> ...]\n\n" +
  ...
  if (results.some((r) => r.kind === "error")) {
    process.stdout.write(
      "Note: some URLs errored (network/timeout). Re-run them to confirm whether\n" +
