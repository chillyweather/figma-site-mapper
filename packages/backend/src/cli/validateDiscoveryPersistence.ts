...
async function main(): Promise<void> {
  ...
  const hasExisting = dbCandidates.some((c) =>
    existingUrls.some((u) => c.normalizedUrl.includes("/products/old-item") || c.normalizedUrl.includes("/blog/existing-post"))
  );
  ...
  check(assertTrue(
    result.recommended.every((candidate) => typeof candidate.id === "number" && candidate.id > 0),
    "recommended result candidates have DB ids"
  ));
