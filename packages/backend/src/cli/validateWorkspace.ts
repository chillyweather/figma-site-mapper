...
async function main(): Promise<void> {
  ...
  // 2. Project manifest
  const projectJson = await readJson<Record<string, unknown>>(path.join(workspacePath, "project.json"));
  if (!projectJson) {
  ...
  const pageDirs = await fs.promises.readdir(pagesDir, { withFileTypes: true }).catch(() => [] as fs.Dirent[]);
  const pageIds: string[] = [];
  for (const entry of pageDirs) {
    if (!entry.isDirectory()) continue;
    const pageId = entry.name;
    pageIds.push(pageId);
    const pageJson = await readJson<unknown>(path.join(pagesDir, pageId, "page.json"));
    if (!pageJson) errors.push(`pages/${pageId}/page.json missing or invalid`);
    const elementsJson = await readJson<unknown>(path.join(pagesDir, pageId, "elements.json"));
    if (!elementsJson) errors.push(`pages/${pageId}/elements.json missing or invalid`);
  }
  ...
  for (const folder of folders) {
    ...
    const groups = await readJson<unknown>(groupsPath);
    if (!groups) errors.push(`catalog/${folder}/groups.json missing or invalid`);
  ...
  for (const { file, schema, label } of decisionValidators) {
    ...
    if (!result.success) {
      errors.push(
        `decisions/${file} invalid: ${result.error.issues.map((i) => `${i.path.join(".")} — ${i.message}`).join("; ")}`
      );
    }
  }
