...
function primaryFontFamily(fontFamily: string | undefined): string | null {
...
function buildFontRow(styles: Styles): ElementPropRow | null {
...
function buildBorderRow(styles: Styles): ElementPropRow | null {
  const style = meaningful(styles, "border-style");
  const width = meaningful(styles, "border-width");
  if (!style || !width || roundPx(width) === "0px") return null;
...
function buildShadowRow(styles: Styles): ElementPropRow | null {
 ...
 */
export function summarizeElementProps(
  styles: Styles | null | undefined,
  bbox?: Bbox | null
): ElementPropRow[] {
  const rows: ElementPropRow[] = [];
  ...
  if (!styles || typeof styles !== "object") return rows;
