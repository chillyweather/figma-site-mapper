...
import type { MappingComponentDecision, MappingPageEvidence } from "../types.js";

// ── helpers ──────────────────────────────────────────────────────────────────

function makePageEvidence(overrides: Partial<MappingPageEvidence> = {}): MappingPageEvidence {
  return {
    id: "10",
...
function makeDecision(overrides: Partial<MappingComponentDecision> = {}): MappingComponentDecision {
  return {
    type: "Buttons",
    instances: [
      {
...
describe("buildRenderComponents", () => {
  ...
  it("returns empty array for empty decisions", () => {
    const pageMap = new Map<string, MappingPageEvidence>();
