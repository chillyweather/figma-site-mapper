...
import {
  effectiveCaptureUrlKey,
  dedupeEffectiveCaptureUrls,
  getUrlLookupCandidates,
} from "./captureUrlNormalization.js";

describe("effectiveCaptureUrlKey", () => {
  it("strips www prefix", () => {
    expect(effectiveCaptureUrlKey("https://www.example.com/")).toBe(
      "https://example.com/"
    );
  });

  it("strips hash fragment", () => {
    expect(effectiveCaptureUrlKey("https://example.com/page#section")).toBe(
      "https://example.com/page"
    );
  });

  it("strips trailing slash from non-root paths", () => {
    expect(effectiveCaptureUrlKey("https://example.com/about/")).toBe(
      "https://example.com/about"
    );
  });

  it("keeps trailing slash on root path", () => {
    expect(effectiveCaptureUrlKey("https://example.com/")).toBe(
      "https://example.com/"
    );
  });

  it("preserves query string", () => {
    expect(effectiveCaptureUrlKey("https://example.com/search?q=test")).toBe(
