...
import type { RepoContextStatus, RepoIndex } from "./types.js";
...
function resolveRequestedBranchRef(repoPath: string, requestedBranch: string): string | null {
  if (!requestedBranch) return null;

  const localRef = runGit(repoPath, ["show-ref", "--verify", "--hash", `refs/heads/${requestedBranch}`]);
  if (localRef) return `refs/heads/${requestedBranch}`;

  const remoteRef = firstNonEmptyLine(
    runGit(repoPath, ["for-each-ref", "--format=%(refname)", `refs/remotes/*/${requestedBranch}`])
  );
  return remoteRef;
}
...
function detectPackageManager(repoPath: string): string | null {
  if (isFile(path.join(repoPath, "pnpm-workspace.yaml"))) return "pnpm";
  if (isFile(path.join(repoPath, "yarn.lock"))) return "yarn";
  if (isFile(path.join(repoPath, "package-lock.json"))) return "npm";
  return isFile(path.join(repoPath, "package.json")) ? "npm-like" : null;
}

function detectUiLibraries(packageJsonFiles: string[]): string[] {
  ...
  for (const filePath of packageJsonFiles) {
    ...
    const deps = {
      ...
      ...(typeof parsed.peerDependencies === "object" && parsed.peerDependencies ? parsed.peerDependencies as Record<string, unknown> : {}),
    };
    const names = Object.keys(deps);
    if (names.some((name) => name === "@mui/material" || name.startsWith("@mui/"))) detected.add("MUI");
    if (names.includes("antd")) detected.add("Ant Design");
    if (names.includes("@chakra-ui/react")) detected.add("Chakra UI");
    if (names.includes("tailwindcss")) detected.add("Tailwind CSS");
    if (names.some((name) => name.startsWith("@radix-ui/"))) detected.add("Radix UI");
    if (names.includes("bootstrap")) detected.add("Bootstrap");
  }

  return Array.from(detected).sort((a, b) => a.localeCompare(b));
}

function detectRoots(repoPath: string, packageRoots: string[]): {
  appRoots: string[];
  componentRoots: string[];
  routeRoots: string[];
} {
...
function detectTokenSources(
  repoPath: string,
  packageJsonFiles: string[],
  configuredTokenSources: string[]
): string[] {
  const detected = new Set<string>();
  ...
  const parentDirs = uniqueSorted(
    packageJsonFiles.map((filePath) => relativeToRepo(repoPath, path.dirname(filePath)))
  );
...
function detectStorybook(repoPath: string): { detected: boolean; roots: string[] } {
  const roots: string[] = [];
...
function sampleComponentFiles(repoPath: string, componentRoots: string[]): string[] {
...
export async function analyzeRepoMappingInput(
  repoPathInput: string,
  requestedBranchInput: string,
  generatedAt: string,
  configuredTokenSources: string[]
): Promise<RepoAdapterResult> {
  ...
  let status: RepoContextStatus = "verified";
  const notes: string[] = [];
  if (!branchExists) {
