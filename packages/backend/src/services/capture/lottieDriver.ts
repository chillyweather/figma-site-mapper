...
export type LottieActivationStatus =
  | "seeked"
  | "warmup"
  | "unsupported"
  | "timeout"
...
export interface LottieActivationResult {
  selector: string;
  status: LottieActivationStatus;
  warning?: string;
}
...
export async function activateLottiePlayers(
  page: Page,
  lottieSelectors: string[]
): Promise<LottieActivationResult[]> {
  if (lottieSelectors.length === 0) return [];

  return page.evaluate(
    async ([selectors, seekProgress, warmupTimeoutMs]: [string[], number, number]) => {
      ...
      for (const sel of selectors) {
        ...
        const candidates = document.querySelectorAll<HTMLElement>(rawSel);
        ...
        const lottieEl = el as HTMLElement & {
          seek?: (progress: number | string) => void;
          pause?: () => void;
          play?: () => void;
          getLottie?: () => {
        ...
        const warmupDone = await new Promise<boolean>((resolve) => {
    ...
    [lottieSelectors, SEEK_PROGRESS, WARMUP_TIMEOUT_MS] as [string[], number, number]
  ) as Promise<LottieActivationResult[]>;
}
