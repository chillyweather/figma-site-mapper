...
export function classifyBlock(input: ClassifierInput): BlockClassification {
  ...
  const cfChallengeSelector =
    html.includes("cf-browser-verification") ||
    html.includes("cf-im-under-attack");
  const cfChallengeTitle =
  ...
  const cfSelectorMatch =
    html.includes("cf-error-details") || html.includes("cf-wrapper");
...
export async function classifyPage(page: Page): Promise<BlockClassification> {
...
function detectCaptchaProvider(html: string): string | null {
  ...
  if (html.includes("shieldsquare")) return "shieldsquare";
