...
export async function scoreSuspiciousRegions(buffer: Buffer): Promise<number> {
  ...
  for (let row = 0; row < height; row++) {
    ...
    let sum = 0;
    for (let i = start; i < start + rowWidth; i++) sum += data[i]!;
    const mean = sum / rowWidth;
    let variance = 0;
    for (let i = start; i < start + rowWidth; i++) variance += (data[i]! - mean) ** 2;
    const stddev = Math.sqrt(variance / rowWidth);

    if (stddev < LOW_VARIANCE_THRESHOLD) {
      currentRun++;
      if (currentRun > maxBlankRun) maxBlankRun = currentRun;
    } else {
      currentRun = 0;
    }
  }
