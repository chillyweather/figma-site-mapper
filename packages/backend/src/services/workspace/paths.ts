...
export function defaultWorkspacePath(projectId: string): string {
...
export function publicAssetUrlToLocalPath(value: string | undefined): string | undefined {
  if (!value) return undefined;

  try {
    const parsed = new URL(value);
    if (!parsed.pathname.startsWith("/screenshots/")) return undefined;
...
export async function linkOrCopyFile(sourcePath: string, targetPath: string): Promise<boolean> {
