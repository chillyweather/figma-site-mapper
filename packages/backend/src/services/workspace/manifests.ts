...
const CATALOG_FOLDERS: WorkspaceCategoryFolder[] = [
  "buttons",
  "links",
  "inputs",
  "headings",
...
function fingerprintFileStem(fingerprint: string): string {
  const hash = crypto.createHash("sha1").update(fingerprint).digest("hex").slice(0, 16);
  return `fp_${hash}`;
}

export function allCatalogFolders(): WorkspaceCategoryFolder[] {
  return CATALOG_FOLDERS;
}

export function categoryCounts(elements: WorkspaceElement[]): Record<string, number> {
...
export function compactStyles(element: WorkspaceElement): Record<string, string> {
  const styles: Record<string, string> = {};
  for (const field of STYLE_FIELDS) {
    const value = getNormalizedStyle(element.styles, field);
    if (value) styles[field] = value;
  }
  return styles;
}

export function buildCatalogGroups(elements: WorkspaceElement[]): Map<WorkspaceCategoryFolder, CatalogGroup[]> {
  const groups = new Map<string, WorkspaceElement[]>();

  for (const element of elements) {
    if (!element.componentFingerprint) continue;
  ...
  const byFolder = new Map<WorkspaceCategoryFolder, CatalogGroup[]>();
  for (const folder of CATALOG_FOLDERS) byFolder.set(folder, []);

  for (const instances of groups.values()) {
    const first = instances[0];
    if (!first?.componentFingerprint) continue;

    const pageIds = Array.from(new Set(instances.map((item) => item.pageId))).sort(
    ...
    const exemplar =
      instances.find((item) => item.cropPath) ??
    ...
    const group: CatalogGroup = {
      ...
      regions: Object.fromEntries(
        Object.entries(regions).sort(([a], [b]) => a.localeCompare(b))
      ),
...
async function linkPageImages(page: WorkspacePage, pageDir: string): Promise<{
...
export async function writePageManifests(
  workspacePath: string,
  data: WorkspaceData
): Promise<void> {
  const catalogDir = path.join(workspacePath, "catalog");
  const elementsByPage = new Map<string, WorkspaceElement[]>();
  ...
  for (const page of data.pages) {
    ...
    await writeJson(path.join(pageDir, "elements.json"), pageElements.map((element) =>
      compactElement(element, pageDir, catalogDir)
    ));
    ...
    await writeJson(path.join(pageDir, "page.json"), {
      ...
      regionSummary: Object.fromEntries(
        Object.entries(regionSummary).sort(([a], [b]) => a.localeCompare(b))
      ),
...
export async function writeCatalogManifests(
  workspacePath: string,
  data: WorkspaceData,
  groupsByFolder: Map<WorkspaceCategoryFolder, CatalogGroup[]>
): Promise<void> {
  const elementsByFolder = new Map<WorkspaceCategoryFolder, WorkspaceElement[]>();
  for (const folder of CATALOG_FOLDERS) elementsByFolder.set(folder, []);
  for (const element of data.elements) {
