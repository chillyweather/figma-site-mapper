...
export function bucketDimension(value: number | undefined): string {
...
export function isLikelyRenderableElement(
  ...
  if (bbox.width > 1600 || bbox.height > 1200) return false;
  return true;
}
...
export function buildClusterSignature(
  ...
  element: ParsedInventoryElement
): { key: string; summary: Record<string, string> } {
  const bbox = element.bbox;
