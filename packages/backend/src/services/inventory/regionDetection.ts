...
function inferTemplateLabel(url: string): string {
  try {
    const pathname = new URL(url).pathname.toLowerCase();
    if (pathname === "/" || pathname === "") return "Home";
    if (/(login|sign-in|signin|sign-up|signup|register)/.test(pathname)) return "Auth";
    if (/(checkout|cart|basket)/.test(pathname)) return "Checkout";
    if (/(account|profile|settings|dashboard)/.test(pathname)) return "Account";
    if (/(product|item)\//.test(pathname)) return "Detail";
    if (/(products|catalog|search|category|categories|blog|docs|articles)/.test(pathname)) {
      return "Listing";
    }

    const segments = pathname.split("/").filter(Boolean);
    if (segments.length >= 2) return "Content Detail";
    return "Content";
  } catch {
    return "Content";
  }
...
export function analyzeRegionsAndTemplates(
  ...
  const regionMap = new Map<
    string,
    {
      pageIds: Set<string>;
      elementCount: number;
      categories: Map<InventoryCategory, number>;
    }
  >();
  ...
  for (const element of elements) {
    ...
    if (!regionMap.has(regionLabel)) {
      regionMap.set(regionLabel, {
        ...
        elementCount: 0,
        categories: new Map<InventoryCategory, number>(),
      });
    }
  ...
  const templateMap = new Map<
    string,
    { pageIds: string[]; sampleUrls: string[] }
  >();
