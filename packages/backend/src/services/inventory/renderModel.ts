...
const TOKEN_CATEGORIES = ["colors", "typography", "spacing", "radii", "shadows"] as const;
type TokenCategory = (typeof TOKEN_CATEGORIES)[number];

const TOKEN_CATEGORY_TITLES: Record<TokenCategory, string> = {
  colors: "Colors",
...
async function loadFrequencyByCategory(
  workspacePath: string
): Promise<Record<TokenCategory, InventoryTokenFrequency[]>> {
  const result = {
    colors: [] as InventoryTokenFrequency[],
    typography: [] as InventoryTokenFrequency[],
    spacing: [] as InventoryTokenFrequency[],
    radii: [] as InventoryTokenFrequency[],
    shadows: [] as InventoryTokenFrequency[],
  };
  for (const category of TOKEN_CATEGORIES) {
    result[category] = await readJsonFile<InventoryTokenFrequency[]>(
      path.join(workspacePath, "tokens", `${category}.json`),
      []
    );
  }
  return result;
...
function collectDecisionValues(token: Record<string, unknown>): string[] {
...
function dedupeOccurrencesByPageClass(
  occurrences: InventoryTokenOccurrence[]
): InventoryTokenOccurrence[] {
  const best = new Map<string, InventoryTokenOccurrence>();
...
function buildSampleLinks(occurrences: InventoryTokenOccurrence[]): RenderLink[] {
...
export async function buildInventoryRenderModel(
  ...
  for (const category of TOKEN_CATEGORIES) {
    ...
    for (const token of decisionTokens) {
      ...
      const matchedOccurrences: InventoryTokenOccurrence[] = [];
      let mergedUsageCount = 0;
      let mergedPageIds = new Set<string>();
      for (const v of decisionValues) {
        const rows = frequencyByValue.get(v) ?? [];
        for (const row of rows) {
          mergedUsageCount += row.usageCount;
          for (const pid of row.pageIds) mergedPageIds.add(pid);
      ...
      if (token.usage && textValue(token.usage)) bodyLines.push(textValue(token.usage));
      if (mergedUsageCount > 0) {
        bodyLines.push(
          `${mergedUsageCount.toLocaleString()} use${mergedUsageCount === 1 ? "" : "s"} on ${mergedPageIds.size} page${mergedPageIds.size === 1 ? "" : "s"}`
        );
      }
